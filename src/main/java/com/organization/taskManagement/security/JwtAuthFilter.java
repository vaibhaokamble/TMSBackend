...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            log.info("Extracted Email from Token: {}", employeeId);
        ...
        if (employeeId != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                ...
                if (jwtService.validateToken(token, userDetails)) {
                    log.info("Token validation successful for: {}", employeeId);
                    ...
                    log.warn("Token validation failed for: {}", employeeId);
                    sendUnauthorizedError(response, "Token validation failed");
                    return;
                }
            } catch (UsernameNotFoundException e) {
                log.warn("User not found: {}", employeeId);
